...
namespace ElggStars\Tests;

use PHPUnit\Framework\TestCase;

/**
 * Auto-generated migration regression guard for plugin "elgg_stars".
 *
...
final class MigrationRegressionTest extends TestCase
{
    ...
    {
        // Scanned with token_get_all(), not a line regex. The original regex ran
        // over raw lines, so any comment that merely *names* unserialize() —
        // e.g. the EncodeSettingsAsJson docblock explaining why the plugin moved
        // off serialize()/unserialize() — was reported as an object-injection
        // RCE. Tokenizing skips comments and strings, and lets the allowed_classes
        // check look at the call's real argument list rather than one source line
        // (a hardened call split across lines would otherwise also be flagged).
        $violations = [];
        foreach (self::files('', 'php') as $file) {
            if (str_contains($file, '/tests/')) {
                continue;
            }
            foreach (self::unserializeCalls((string) file_get_contents($file)) as [$line, $args]) {
                if (!str_contains($args, 'allowed_classes')) {
                    $violations[] = basename($file) . ':' . $line . ' unserialize() without allowed_classes => false (PHP object-injection RCE)';
                }
            }
        }
    ...
    /**
     * Locate real unserialize() call sites. Returns [line, argument-list source] pairs.
     *
     * Method calls ($obj->unserialize()), static calls and declarations
     * (function unserialize()) are not the global function and are skipped.
     */
    private static function unserializeCalls(string $src): array
    {
        ...
        foreach ($tokens as $i => $token) {
            ...
            if (is_array($prev) && in_array($prev[0], [T_OBJECT_OPERATOR, T_NULLSAFE_OBJECT_OPERATOR, T_DOUBLE_COLON, T_FUNCTION, T_NEW], true)) {
                continue;
            }
